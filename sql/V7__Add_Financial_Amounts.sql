-- V7: Add Financial Amount Columns and Sync Data
...
-- 3. Run Data Sync (From transaction tables to ledgers)

-- 3.1 Sync FTO Amounts
WITH fto_daily AS (
    SELECT 
        financial_year, 
        ddo_code, 
        userid, 
        (fto_creation_date AT TIME ZONE 'UTC')::date as ledger_date,
        SUM(amount) as daily_fto_amount
    FROM fto.fto_list
    GROUP BY 1, 2, 3, 4
)
UPDATE dashboard.daily_ledger_operator dl
SET fto_amount = fd.daily_fto_amount
FROM fto_daily fd
WHERE dl.financial_year = fd.financial_year 
  AND dl.ddo_code = fd.ddo_code 
  AND dl.userid = fd.userid 
  AND dl.ledger_date = fd.ledger_date;

-- 3.2 Sync Bill Amounts
WITH bill_daily AS (
    SELECT 
        financial_year, 
        ddo_code, 
        userid, 
        (bill_date AT TIME ZONE 'UTC')::date as ledger_date,
        SUM(amount) as daily_bill_amount
    FROM bills.bill_list
    GROUP BY 1, 2, 3, 4
)
UPDATE dashboard.daily_ledger_operator dl
SET bill_amount = bd.daily_bill_amount
FROM bill_daily bd
WHERE dl.financial_year = bd.financial_year 
  AND dl.ddo_code = bd.ddo_code 
  AND dl.userid = bd.userid 
  AND dl.ledger_date = bd.ledger_date;

-- 3.3 Sync Forwarded Amounts (Best effort using bill_date as log is unavailable)
WITH forward_daily AS (
    SELECT 
        financial_year,
        ddo_code,
        (bill_date AT TIME ZONE 'UTC')::date as ledger_date,
        SUM(amount) as daily_forward_amount
    FROM bills.bill_list
    WHERE bill_status >= 4 -- Forwarded to Treasury
    GROUP BY 1, 2, 3
)
UPDATE dashboard.daily_ledger_approver dl
SET forwarded_amount = fd.daily_forward_amount
FROM forward_daily fd
WHERE dl.financial_year = fd.financial_year 
  AND dl.ddo_code = fd.ddo_code 
  AND dl.ledger_date = fd.ledger_date;

-- 3.4 Roll up to Admin and Approver Ledgers
UPDATE dashboard.daily_ledger_admin dl
SET 
...
-- 4. Trigger Hardening to update summary tables
